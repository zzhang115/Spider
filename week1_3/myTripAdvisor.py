...
def getContentFromSingleURL(url):
    ...
    # print(webdata.text)
    # pc browser parse source
    titles = soup.select('div.listing_info > div.listing_title')#[target="_blank"]')
    images = getImageFakeAsIphone(url)  #soup.select('div.attraction_clarity_cell > div > div > div.photo_booking.non_generic > a.photo_link > img')
